...
async fn raw_json_rpc_initializes_lists_tools_and_returns_a_validation_error() {
    ...
    let inbox_tool = tools["result"]["tools"]
        .as_array()
        .unwrap()
        .iter()
    ...
    for expected in ["truncation_reason", "conversation_limit", "byte_limit"] {
        assert!(
            inbox_output_schema.contains(expected),
    ...
    let render_tool = tools["result"]["tools"]
        .as_array()
        .unwrap()
        .iter()
        .find(|tool| tool["name"] == "slack_render_markdown")
        .unwrap();
    let render_output_schema = serde_json::to_string(&render_tool["outputSchema"]).unwrap();
    for expected in [
        "outbound_mentions",
        "reference",
        "user_id",
        ...
        assert!(
            render_output_schema.contains(expected),
            "render output schema omits {expected}"
        );
    }
